...
export type ScheduledFixture = Fixture & {
  homeTeam: Team;
  awayTeam: Team;
};
...
function requireDb() {
  return getDb().then((db) => {
    if (!db) {
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database is not available." });
    }
    return db;
  });
}
...
export async function listLeagues() {
  ...
  return leagueRows.map((league) => ({
    ...
    completedFixtureCount: fixtureRows.filter(
      (fixture) => fixture.leagueId === league.id && fixture.status === "completed"
    ).length,
  }));
}

export async function getLeagueDashboard(leagueId: number) {
  const db = await requireDb();
  ...
  return {
    ...
    standings: calculateStandings(teamRows, fixtureRows),
  };
}
...
export async function createLeague(input: {
  ...
  const db = await requireDb();
  const result = await db.insert(leagues).values(input);
...
export async function addTeam(input: {
  ...
  const existing = await db
    ...
    .limit(1);
  if (existing[0]) {
    throw new TRPCError({ code: "CONFLICT", message: "A team with this name is already registered in the league." });
  }
...
export async function updateTeam(input: {
  ...
  const db = await requireDb();
  const teamRows = await db.select().from(teams).where(eq(teams.id, input.teamId)).limit(1);
  ...
  const duplicate = await db
    .select({ id: teams.id })
    .from(teams)
    .where(and(eq(teams.leagueId, team.leagueId), eq(teams.name, input.name)))
    .limit(1);
  if (duplicate[0] && duplicate[0].id !== input.teamId) {
    throw new TRPCError({ code: "CONFLICT", message: "A team with this name is already registered in the league." });
  }
...
export async function removeTeam(teamId: number) {
  const db = await requireDb();
  const fixtureRows = await db.select({ id: fixtures.id }).from(fixtures).where(
    or(eq(fixtures.homeTeamId, teamId), eq(fixtures.awayTeamId, teamId))
  ).limit(1);
  if (fixtureRows[0]) {
    throw new TRPCError({ code: "PRECONDITION_FAILED", message: "This team is already in the fixture schedule. Remove the schedule before deleting it." });
  }
  await db.delete(teams).where(eq(teams.id, teamId));
  return { success: true } as const;
}

export async function generateFixtures(leagueId: number) {
  const db = await requireDb();
  ...
  if (teamRows.length < 2) {
    throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Register at least two teams before generating fixtures." });
  }
  if (existingFixtures[0]) {
    throw new TRPCError({ code: "CONFLICT", message: "Fixtures have already been generated for this league." });
  }

  const schedule = generateRoundRobinFixtures(teamRows.map((team) => team.id));
  if (schedule.length) {
    await db.insert(fixtures).values(schedule.map((fixture) => ({ ...fixture, leagueId })));
  }
...
export async function recordResult(input: {
  ...
  if (fixture.status === "completed") {
    throw new TRPCError({ code: "CONFLICT", message: "This fixture already has a recorded result." });
  }
